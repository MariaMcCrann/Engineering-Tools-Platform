...
export function BoxCulvertLoadGenerator({onApply}:{onApply:(r:BoxLoadResult)=>void}){
  ...
  useEffect(()=>{onApply(r);},[r,onApply]);
  return <section className="upload-card"><div className="card-head"><div><h2>Box Culvert Loading</h2><p>Workbook SM1600 loading chain → base-slab design actions</p></div><span className="valid">✓ Auto-linked</span></div><div className="form-grid">
  ...
  </div><section className="model-strip"><div><span>ULS M+</span><strong>{fmt(r.Mp)} kNm/m</strong></div><div><span>ULS M−</span><strong>{fmt(r.Mn)} kNm/m</strong></div><div><span>ULS shear</span><strong>{fmt(r.V)} kN/m</strong></div><div><span>SLS M+</span><strong>{fmt(r.Msls)} kNm/m</strong></div><div><span>Bearing</span><strong>{fmt(r.bearing)} kPa</strong></div><div><span>Slab</span><strong>{fmt(r.slabWidth,2)} × {fmt(r.slabLength,3)} m</strong></div></section><p className="subtitle">These calculated dimensions and actions automatically feed the concrete section below.</p></section>;
}
...
export type HeadwallActionResult={length:number;height:number;hMp:number;hMn:number;hV:number;vMp:number;vMn:number;vV:number;ratio:number;deadPressure:number};
export function calculateHeadwallActions(length:number,height:number,soilHeight:number,waterHeight:number,soilDensity:number,waterDensity:number,livePressure:number):HeadwallActionResult{
  const buoyant=soilDensity-waterDensity;const deadPressure=buoyant*soilHeight*0.5+waterDensity*waterHeight;const b=height;
  ...
  return{length,height,hMp:Math.max(...mx),hMn:Math.min(...mx),hV:Math.max(...rx),vMp:Math.max(...my),vMn:Math.min(...my),vV:Math.max(...ry),ratio:length/height,deadPressure};
}
