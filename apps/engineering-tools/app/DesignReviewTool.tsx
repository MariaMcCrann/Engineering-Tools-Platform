...
import { download, filename, reportHtml, wordDocument, workbook } from "./design-review/exports";
...
export function DesignReviewTool() {
  ...
  return <div className="review-workspace">
    ...
    {review && !form && <>
      ...
      {section === "audit" ? <section className="review-card"><h2>Audit trail</h2><p>Append-only within this app. Local files are editable outside the app; this is not a tamper-proof or authenticated audit system.</p>{[...review.audit].reverse().map(e => <details key={e.id} className="review-audit"><summary>{stamp(e.at)} · {e.actor.name} · {e.action}</summary><pre>{e.detail}</pre></details>)}</section> : <>
