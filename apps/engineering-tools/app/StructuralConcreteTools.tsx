...
function sectionChecks(D:number,cover:number,db:number,spacing:number,mesh:string,meshDir:"pri"|"sec",fc:number,fsy:number,moment:number,serviceMoment:number,shrinkMu:number){
  ...
  const p=Ast/(1000*d);const n=ES/Ec;const np=n*p;const k=Math.sqrt(np*np+2*np)-np;const kd=k*d;
...
function ReinforcementFields({prefix,db,setDb,spacing,setSpacing,mesh,setMesh}:{prefix:string;db:number;setDb:(n:number)=>void;spacing:number;setSpacing:(n:number)=>void;mesh:string;setMesh:(s:string)=>void}){
  return <><label>{prefix} N-bar<select value={db} onChange={e=>setDb(+e.target.value)}>{BARS.map(x=><option key={x} value={x}>{x?`N${x}`:"None"}</option>)}</select></label><label>{prefix} bar spacing (mm)<input type="number" value={spacing} onChange={e=>setSpacing(+e.target.value)}/></label><label>{prefix} mesh<select value={mesh} onChange={e=>setMesh(e.target.value)}>{Object.keys(MESH).map(x=><option key={x}>{x}</option>)}</select></label></>;
}
...
export function HeadwallStructuralTool(){
  ...
  const applyHeadwall=useCallback((x:{length:number;height:number;hMp:number;hMn:number;hV:number;vMp:number;vMn:number;vV:number})=>{
    setL(x.length*1000);setH(x.height*1000);
    setHMp(x.hMp);setHMn(x.hMn);setHV(x.hV);setVMp(x.vMp);setVMn(x.vMn);setVV(x.vV);
    setHMsls(x.hMp*0.7);setVMsls(x.vMp*0.7);
  },[]);
  ...
  const neg=useMemo(()=>sectionChecks(D,cover,0,300,mesh,"pri",fc,fsy,Math.max(Math.abs(hMn),Math.abs(vMn)),Math.max(hMsls*0.12,vMsls*0.16),1.75),[D,cover,mesh,fc,fsy,hMn,vMn,hMsls,vMsls]);
  ...
  const checks:Check[]=[
    ...
    {label:"Horizontal -ve bending φMuo",demand:Math.abs(hMn),capacity:neg.phiMuo,unit:"kNm/m",pass:Math.abs(hMn)<=neg.phiMuo},
    {label:"Vertical -ve bending φMuo",demand:Math.abs(vMn),capacity:neg.phiMuo,unit:"kNm/m",pass:Math.abs(vMn)<=neg.phiMuo},
  ...
  return <div className="content"><div className="title-row"><div><p className="eyebrow">STRUCTURES · AS 3600 WORKBOOK PORT</p><h1>Headwall Concrete</h1><p className="subtitle">Generate the USBR plate actions and automatically check the reinforced-concrete wall section using the supplied AS 3600:2018 workbook equations.</p></div></div>
    <HeadwallLoadGenerator onApply={applyHeadwall}/>
    <h2>Concrete section</h2><div className="form-grid"><label>Wall length (mm) · from plate model<input type="number" value={L} readOnly/></label><label>Wall height (mm) · from plate model<input type="number" value={H} readOnly/></label><label>Wall thickness D (mm)<input type="number" value={D} onChange={e=>setD(+e.target.value)}/></label><label>Concrete f'c (MPa)<select value={fc} onChange={e=>setFc(+e.target.value)}>{Object.keys(FC_EC).map(x=><option key={x}>{x}</option>)}</select></label><label>Steel fsy (MPa)<input type="number" value={fsy} onChange={e=>setFsy(+e.target.value)}/></label><label>Cover (mm)<input type="number" value={cover} onChange={e=>setCover(+e.target.value)}/></label><ReinforcementFields prefix="Inside face" db={db} setDb={setDb} spacing={spacing} setSpacing={setSpacing} mesh={mesh} setMesh={setMesh}/></div>
    <h2>Design actions <small>· automatically calculated above</small></h2><div className="form-grid"><label>Horizontal M+ (kNm/m)<input type="number" value={hMp} readOnly/></label><label>Horizontal M− (kNm/m)<input type="number" value={hMn} readOnly/></label><label>Horizontal V* (kN/m)<input type="number" value={hV} readOnly/></label><label>Vertical M+ (kNm/m)<input type="number" value={vMp} readOnly/></label><label>Vertical M− (kNm/m)<input type="number" value={vMn} readOnly/></label><label>Vertical V* (kN/m)<input type="number" value={vV} readOnly/></label><label>Horizontal service M (kNm/m)<input type="number" value={hMsls} readOnly/></label><label>Vertical service M (kNm/m)<input type="number" value={vMsls} readOnly/></label></div>
    ...
    <div className="warning"><div>i</div><p><strong>USBR coefficient basis:</strong> the supplied workbook contains the Case 4 coefficient set at a/b ≈ 1.116. The generator flags dimensions that move materially away from that aspect ratio. The shear value remains a screening check; use the workbook’s full variable-angle shear procedure where transverse reinforcement is required.</p></div><CheckTable checks={checks}/>
  </div>;
}
...
export function BaseSlabStructuralTool(){
  ...
  const applySlab=useCallback((x:{slabLength:number;slabWidth:number;Mp:number;Mn:number;V:number;Msls:number;bearing:number})=>{setLength(x.slabLength*1000);setWidth(x.slabWidth*1000);setMp(x.Mp);setMn(x.Mn);setV(x.V);setMsls(x.Msls);setBearing(x.bearing);},[]);
  const r=useMemo(()=>sectionChecks(D,cover,db,spacing,mesh,"pri",fc,fsy,Mp,Msls,1.75),[D,cover,db,spacing,mesh,fc,fsy,Mp,Msls]);
  const neg=useMemo(()=>sectionChecks(D,cover,db,spacing,mesh,"pri",fc,fsy,Math.abs(Mn),Math.abs(Mn)*0.7,1.75),[D,cover,db,spacing,mesh,fc,fsy,Mn]);
  const phiVucApprox=0.7*0.16*1000*(0.9*r.d)*Math.sqrt(fc)/1000;
  const checks:Check[]=[
    {label:"Positive bending φMuo",demand:Mp,capacity:r.phiMuo,unit:"kNm/m",pass:Mp<=r.phiMuo},
    {label:"Negative bending φMuo",demand:Math.abs(Mn),capacity:neg.phiMuo,unit:"kNm/m",pass:Math.abs(Mn)<=neg.phiMuo},
  ...
  return <div className="content"><div className="title-row"><div><p className="eyebrow">STRUCTURES · AS 3600 WORKBOOK PORT</p><h1>Base Slab Concrete</h1><p className="subtitle">Run the box-culvert loading model and automatically feed its geometry and actions into the AS 3600 slab section check.</p></div></div>
    <BoxCulvertLoadGenerator onApply={applySlab}/>
    <h2>Concrete section</h2><div className="form-grid"><label>Slab length (mm) · from loading model<input type="number" value={length} readOnly/></label><label>Slab width (mm) · from loading model<input type="number" value={width} readOnly/></label><label>Slab depth D (mm)<input type="number" value={D} onChange={e=>setD(+e.target.value)}/></label><label>Concrete f'c (MPa)<select value={fc} onChange={e=>setFc(+e.target.value)}>{Object.keys(FC_EC).map(x=><option key={x}>{x}</option>)}</select></label><label>Steel fsy (MPa)<input type="number" value={fsy} onChange={e=>setFsy(+e.target.value)}/></label><label>Cover (mm)<input type="number" value={cover} onChange={e=>setCover(+e.target.value)}/></label><ReinforcementFields prefix="Top / bottom face" db={db} setDb={setDb} spacing={spacing} setSpacing={setSpacing} mesh={mesh} setMesh={setMesh}/></div>
    <h2>Design actions <small>· automatically calculated above</small></h2><div className="form-grid"><label>Positive M* (kNm/m)<input type="number" value={Mp} readOnly/></label><label>Negative M* (kNm/m)<input type="number" value={Mn} readOnly/></label><label>V* (kN/m)<input type="number" value={V} readOnly/></label><label>Service M (kNm/m)<input type="number" value={Msls} readOnly/></label><label>Required soil bearing (kPa)<input type="number" value={bearing} readOnly/></label></div>
    ...
    <div className="warning"><div>i</div><p><strong>Integrated loading:</strong> the generator ports the workbook’s SM1600 S/M axle interpolation, dynamic allowance, soil-depth reductions, dead load, two-lane bearing pressure and one/two/three-cell slab action coefficients. T44 is still reference data in the original sheet and is not used by its design-action formulas.</p></div><CheckTable checks={checks}/>
  </div>;
}
