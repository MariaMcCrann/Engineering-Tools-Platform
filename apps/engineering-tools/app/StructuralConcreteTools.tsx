...
export function HeadwallStructuralTool(){
  ...
  return <div className="content"><div className="title-row"><div><p className="eyebrow">STRUCTURES · AS 3600 WORKBOOK PORT</p><h1>Headwall Concrete</h1><p className="subtitle">Generate the USBR plate actions, then check the reinforced-concrete wall section using the supplied AS 3600:2018 workbook equations.</p></div></div>
    <HeadwallLoadGenerator onApply={x=>{setHMp(x.hMp);setHMn(x.hMn);setHV(x.hV);setVMp(x.vMp);setVMn(x.vMn);setVV(x.vV);}}/>
    <h2>Concrete section</h2><div className="form-grid"><label>Wall length (mm)<input type="number" value={L} onChange={e=>setL(+e.target.value)}/></label><label>Wall height (mm)<input type="number" value={H} onChange={e=>setH(+e.target.value)}/></label><label>Wall thickness D (mm)<input type="number" value={D} onChange={e=>setD(+e.target.value)}/></label><label>Concrete f'c (MPa)<select value={fc} onChange={e=>setFc(+e.target.value)}>{Object.keys(FC_EC).map(x=><option key={x}>{x}</option>)}</select></label><label>Steel fsy (MPa)<input type="number" value={fsy} onChange={e=>setFsy(+e.target.value)}/></label><label>Cover (mm)<input type="number" value={cover} onChange={e=>setCover(+e.target.value)}/></label><ReinforcementFields prefix="Inside face" db={db} setDb={setDb} spacing={spacing} setSpacing={setSpacing} mesh={mesh} setMesh={setMesh}/></div>
...
export function BaseSlabStructuralTool(){
  ...
  return <div className="content"><div className="title-row"><div><p className="eyebrow">STRUCTURES · AS 3600 WORKBOOK PORT</p><h1>Base Slab Concrete</h1><p className="subtitle">Run the box-culvert loading model and transfer its actions directly into the AS 3600 slab section check.</p></div></div>
    <BoxCulvertLoadGenerator onApply={x=>{setLength(x.slabLength*1000);setWidth(x.slabWidth*1000);setMp(x.Mp);setMn(x.Mn);setV(x.V);setMsls(x.Msls);setBearing(x.bearing);}}/>
