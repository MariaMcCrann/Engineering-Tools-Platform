...
export function PipelineHglTool() {
  ...
  const exportCsv = () => {
    const rows = [["Reach", "Length (m)", "Material", "Nominal (mm)", "ID (mm)", "Barrels", "K", "Velocity (m/s)", "Pipe roughness (mm)", "hf (mm)", "hs (mm)", "Total loss (mm)", "Downstream HGL (m AHD)", "Upstream HGL (m AHD)"],
      ...results.map((x) => x.valid ? [x.name, x.length, x.material, x.nominal, x.idMm, x.barrels, x.k, x.velocity, ROUGHNESS_MM[x.material], x.hf * 1000, x.hs * 1000, x.total * 1000, x.downstreamHgl, x.upstreamHgl] : [x.name, x.length, x.material, x.nominal, "INVALID"] )];
